...
| `q8_checkpoint_670.json` | A recovered 670-edge intermediate solution. Rename to `q8_best.json` and run `sa_q8.py` in the same directory to reproduce our finding that the code makes genuine progress (90 seconds of run time reduces the violation count at the 675-edge target from 22 to 8) -- without this file, `sa_q8.py` instead starts from a fresh greedy construction. |
