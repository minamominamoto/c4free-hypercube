...
have symmetric $U$/complement degree distributions; the identity
appears tied to that symmetry (which the odd-square condition implies
but does not require) rather than to odd-squareness itself, and we do
not characterise the identity's full necessary-and-sufficient condition
here.
...
is equivalent to maximising $\sum_{v\in U}h(v)$ subject to a \emph{fixed}
sum of squares $n\cdot2^{n-1}$ over the $2^{n-1}$ nonnegative integers
$h(v)$ of fixed parity. By the QM–AM inequality, for a fixed sum of
squares the linear sum is maximised when the values are as equal as
possible, i.e.\ concentrated on the integers of the correct parity
nearest $\sqrt n$ (matching Derrida et al.'s own description of the
extremal distribution~\cite{DPTV79}). For $n=7$ ($\sqrt7\approx2.65$,
odd values), concentrating on $\{1,3\}$ and solving
$n_1+n_3=64$, $n_1+9n_3=448$ gives $n_3=48$, $n_1=16$, so
$\sum_{v\in U}h(v)=16\cdot1+48\cdot3=160$ and $|E|=(448+160)/2=304$. For
$n=8$ ($\sqrt8\approx2.83$, even values), a pure two-value
$\{2,4\}$-concentration is not even integrally feasible here: solving
$n_2+n_4=128$, $4n_2+16n_4=1024$ gives $n_4=128/3$, not an integer. The
extremal distribution for $n=8$ genuinely needs a third value; the
released 682-edge witness (Section~\ref{sec:oddsquareverify}) has
$h$-distribution on $U$ equal to $\{0{:}1,\,2{:}84,\,4{:}43\}$
(independently recomputed from the released edge list), giving
$\sum_{v\in U}h(v)=0\cdot1+2\cdot84+4\cdot43=340$ and matching
$\sum_{v\in U}h(v)^2=0+336+688=1024$ exactly, but we do not derive from
first principles here that this three-value distribution is optimal
among all integer distributions satisfying Lemma~\ref{lem:fieldbound}'s
constraint; that optimality is DPTV's Table~I result, taken as external
input, consistent with their own remark that for $d=6,8$ the extremal
distribution is not two-valued and not unique unlike other $d\le7$. The
$n=7$ case above is, however, fully self-contained given
Lemma~\ref{lem:fieldbound}.
