...
$p=(n+h(v))/2$ to $q=(n-h(v))/2$, a change of $q-p=-h(v)>0$; no other
vertex's incident edges change sign, so $|E|$ increases by exactly
$-h(v)>0$. This contradicts maximality of $|E|$, so no such $v$ exists.
\end{proof}
...
manifest); we do not supply a separate
document listing which specific $Q_6$ edge each $x_i$ corresponds to,
so a reader wishing to interpret an individual solution vector edge-by-edge
would need to infer that correspondence from the generator (not released
separately) or from the constraint incidence pattern itself. This does
not affect the feasible value (132, matching the released construction)
or the structural checks (192 variables, 240 constraints, 5-regular
incidence), which we verified directly from the MPS file's own contents.
