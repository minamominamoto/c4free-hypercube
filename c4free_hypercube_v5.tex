...
The original ($Q_7$, $Q_8$) constructions are reported to have been found by a
two-phase simulated annealing algorithm, described in full detail; the
released search code computes an $\mathrm{Aut}(Q_n)$-diversified
restart point before each trial but does not pass it into the phase-1
search, which always restarts from a fresh uniform-random edge set
instead. On direct execution, however, we found that the released,
unmodified phase-1 code frequently cannot make any progress at all
from such a fresh random start under its own documented parameter
ranges, for a structural reason unrelated to step count or luck of
the seed (Section~\ref{sec:method}); we could not identify parameters
under which it reaches these targets with zero violations. The
released edge-set certificates themselves remain independently
verified as valid; what we cannot confirm is that the released,
unmodified search code -- run as documented -- is what actually
produced them. For $Q_6$ we additionally give an ILP formulation whose
...
We report having constructed a $C_4$-free subgraph of $Q_7$ on 304 edges by
penalty-based simulated annealing (Section~\ref{sec:method}; see that
section for a caveat about the released code's ability to reproduce
this process),
...
Through $19\,866$ reported simulated-annealing runs (Section~\ref{sec:landscape};
see Section~\ref{sec:method} for what is and is not independently
verifiable about this search history)
...
This section documents the originally announced $Q_8$ witness, reported to
have been obtained independently of the odd-square subclass by the same
simulated-annealing method as the $Q_7$ construction of Section~\ref{sec:q7}
(see Section~\ref{sec:method} for the finding that the released,
unmodified code cannot be confirmed to reproduce this process). It has since been
...
independent structural data point, not as a demonstration that the
simulated-annealing method as released is capable of finding it.

Simulated annealing is reported to have produced two $C_4$-free subgraphs of $Q_8$ on 680
...
the resulting change in $|E|$ directly.
...
``the improved lower bound at $D=8$ gives $-361$.'' Applying the same
factor to our value gives $768\times(-0.4696\ldots)=-360.62\ldots
...
$19\,866$ solutions reported found by simulated annealing (Section~\ref{sec:q7}),
...
Source & one of 389 reported SA sols. & witness for~\cite{MPR95} bound\\
...
statistics noted earlier, not via a bundled analysis script). The quantitative
claims about the \emph{search process} that
produced the solutions — ``$19\,866$ independent runs'' and ``$1\,076$ independent
trials'' at $305$ and at $681$ edges — describe unarchived search history. The
...
the same sense as the edge sets. Worse, as detailed in
Section~\ref{sec:method}'s Diversification discussion below, we found
...
reader wishing to verify the exact process-level statistics should
therefore not assume rerunning the released code as documented will
reproduce them, or indeed complete a single trial successfully; see
Section~\ref{sec:method} for the specifics and for what remains an
open provenance question about the actual search method used.

Beyond the aggregate counts, each released $Q_7$ record itself carries
undocumented provenance fields (\texttt{method}, \texttt{run},
\texttt{trial}, \texttt{hash}, \texttt{elapsed}) that we did not
explain in earlier versions of this manuscript. For completeness: of
...
We do not know what distinction \texttt{auto} vs.\ \texttt{auto\_seed}
vs.\ \texttt{repair} marks, what unit of independence \texttt{run} or
\texttt{trial} refers to, or why only $8$ records carry a \texttt{run}
field, since this was not documented at the time of the original
search and we do not want to guess retrospectively. These fields
should therefore be treated as additional unexplained search-history
metadata, in the same category as the process-level claims discussed
above, not as a resolved provenance record.
