...
Given Lemma~\ref{lem:fieldbound} and $h(v)\ge0$, maximising $|E|=(n\cdot2^{n-1}+\sum_{v\in U}h(v))/2$
is equivalent to maximising $\sum_{v\in U}h(v)$ subject to the \emph{fixed}
sum of squares $\sum_{v\in U}h(v)^2=n\cdot2^{n-1}$ over the $2^{n-1}$
nonnegative integers $h(v)$ of fixed parity. Both cases $n=7,8$ admit a
