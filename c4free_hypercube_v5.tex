...
reconstructed it (\texttt{q6\_ilp\_edge\_map.csv}: $x_i\leftrightarrow$
vertex pair) by matching the MPS's variable--constraint incidence
pattern against $Q_6$'s known edge--square incidence structure (squares
enumerated by base vertex, then direction pair), and verified the
reconstruction exactly reproduces all $240$ constraints' variable
memberships from the released MPS file.
