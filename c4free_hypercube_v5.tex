...
spin configuration $s$ range over all $2^n$ choices produces exactly the
...
The distribution's nonnegativity, $h(v)\ge0$, is not implied by
Lemma~\ref{lem:fieldbound} alone; it is the extra ground-state
condition of Derrida et al.~\cite{DPTV79} (a configuration with some
$h(v)<0$ cannot be minimal, since flipping that vertex's spin strictly
reduces the number of negative edges without violating odd-squareness).
We take this as an external fact from~\cite{DPTV79} rather than
reproving it here.

Given Lemma~\ref{lem:fieldbound} and $h(v)\ge0$, $|E|=(n\cdot2^{n-1}+\sum_{v\in U}h(v))/2$
...
from Lemma~\ref{lem:fieldbound} alone; the explicit witness realising it
