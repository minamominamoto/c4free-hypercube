...
- Orbit-witness certificate: `q7_orbit_witnesses.json` (generated by
  ...
  library, ~16 s) verifies every one of the 19,866 witness group
  elements, the identity of the certified assignment with
  `q7_orbit_census.json`, the attainment of each orbit's stored
  canonical form, and the pairwise distinctness of the 180 canonical
  forms — certifying the assignment ("at most 180 orbits") with no group
  sweep. `--canonical [--orbits A:B]` (numpy, census-scale) re-verifies
  each canonical form is the true orbit minimum; minimality plus
  distinctness certifies "exactly 180" independently of the census scan.
  Wired into `reproduce_core.py`: default mode in the default path,
...
- `cross_verify.py`: bundled independent re-implementation of the
  ...
  Hamming-distance-2 pairs; sorted-tuple distinctness, no hashing;
  no code shared with `verify.py`). Standard library, ~30 s; in the
  default path. Makes the manuscript's single-implementation-risk
  mitigation auditable.
- Manuscript round-79 changes: the abstract is rewritten to summary
  length and the material it carried (attribution detail, verification
  map, provenance record) is relocated verbatim into a new Section 2
  "Reader's Guide: Scope, Attribution, and Provenance"; thm:main is
  retitled "Certified lower bounds" with a certificate-form lead-in; the
  n=4 table entry now carries a two-line proof (24 squares / each edge in
  3 of them => at least 8 edges omitted) with the matching construction
  cited from Brass–Harborth–Nienborg; the DPTV79 plural quotation now
  also appears at the "both non-realisable distributions" claim; the
  seed-90008 byte-identity claim is scoped to the reference platform
  class (floating-point exponential in the acceptance step); an
  explicit note that we know of no published independent re-verification
  of HN94's combinatorial proof; the average-split condition is marked
  diagnostic (not a search filter); "locally maximal" is defined at first
  Q8 use as maximality under edge addition; the profile table caption
  states the non-increasing canonical ordering; Section 7 opens with a
  positioning sentence (independent of Theorems 1–2 except the 389
  audit); and the census discussion documents the new certificate route.
